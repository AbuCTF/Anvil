<script lang="ts">
  ...
  const draw = (pts: { x: number; y: number }[]) =>
    curve === "step" ? stepPath(pts) : monotonePath(pts);
  ...
  $: tickCount = width < 480 ? 3 : 4;
  $: xTicks = xFormat
    ? Array.from(
        { length: tickCount + 1 },
        (_, index) => xMin + (index / tickCount) * (xMax - xMin),
      )
    : [];
  ...
  $: dataX =
    hoverX == null
      ? null
      : xMin +
        ((hoverX - padL) / Math.max(1, width - padR - padL)) * (xMax - xMin);
  $: hoverRows =
    dataX == null
      ? []
      : series
  ...
  $: tipLeft =
    hoverX == null
      ? 0
      : Math.min(Math.max(0, hoverX + 12), Math.max(0, width - 172));

  function onMove(e: PointerEvent) {
    const rect = (e.currentTarget as SVGElement).getBoundingClientRect();
    const scaledX = ((e.clientX - rect.left) / Math.max(1, rect.width)) * width;
    hoverX = Math.max(padL, Math.min(width - padR, scaledX));
  }
</script>
...
<div class="relative w-full" bind:clientWidth={width}>
  ...
  >
    {#each yTicks as t}
      ...
      <text
        ...
        y={sy(t) + 3}
        text-anchor="end"
        class="fill-stone-600 text-[10px] tabular-nums">{t}</text
      >
    {/each}
    ...
    {#each paths as p}
      <path
        ...
        stroke-opacity={p.dim ? "var(--chart-dim-opacity)" : 1}
  ...
  {#if hoverX != null && hoverRows.length}
    ...
    >
      {#if xFormat && dataX != null}<div
          class="text-stone-500 mb-1 tabular-nums"
        >
          {xFormat(dataX)}
        </div>{/if}
      {#each hoverRows as r}
        <div class="flex items-center gap-1.5 leading-none">
          ...
          <span class="optical-label ml-auto text-stone-400 tabular-nums"
            >{Math.round(r.value ?? 0)}</span
          >
        </div>
      {/each}
    </div>
